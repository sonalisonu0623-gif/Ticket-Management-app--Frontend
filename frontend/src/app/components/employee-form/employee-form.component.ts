...
export class EmployeeFormComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    req.subscribe({
      next: () => {
        ...
        setTimeout(() => this.router.navigate(['/configuration'], { queryParams: { tab: 'employees' } }), 1500);
      },
      error: err => {
        this.submitting = false;
  ...
  onReset(): void {
    ...
    else { this.employeeForm.reset({ status: 'ACTIVE', role: 'EMPLOYEE' }); this.selectedProjectIds = []; }
