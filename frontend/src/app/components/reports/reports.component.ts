...
export class ReportsComponent implements OnInit {
  ...
  filterForm = this.fb.group({ projectId: [null as number | null] });
  ...
  readonly priorityEntries = computed(() => {
    ...
    return Object.entries(d.ticketsByPriority ?? {}).sort((a, b) => order.indexOf(a[0]) - order.indexOf(b[0]));
  });
  ...
  maxStatus(): number { return Math.max(...this.statusEntries().map(([,v]) => v), 1); }
  maxPriority(): number { return Math.max(...this.priorityEntries().map(([,v]) => v), 1); }
}
